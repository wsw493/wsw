...
function Org({dispatch,org}){
    ...
    const mapCallback = (lt,lglt)=>{
        ...
        dispatch({type:'org/updateNewItem',payload:{address:lglt}});
        dispatch({type:'org/updateEditItem',payload:{address:lglt}});
    }
    const closeMapModal = ()=>{
        dispatch({type:'org/updateState',payload:{modal1Visible:false}});
    }
		...
		<div className={styles.main}>
			...
			<div className={styles.rigthCont}>
	            ...
	            <div className={styles.bt_bar}>
	                {functionCodesMap['CF_MANAGE_TENANT_ORG_ADD']?<Button icon="file-add" className='primary' onClick={showNewWindow}>新增</Button>:''}
	            </div>
