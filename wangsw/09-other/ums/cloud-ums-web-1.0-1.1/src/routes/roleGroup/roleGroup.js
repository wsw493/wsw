...
import RoleGroupTree from '../../components/roleGroup/roleGroupTree';
import VtxPopconfirm from '../../components/vtxCommon/vtxPopconfirm';

function RoleGroup({dispatch,roleGroup}) {
	const {tableData,tableLoading,pageSize,currentPageIndex,newItem,editItem,viewItem,
		selectedRowKeys,saving,totalItems,treeData,searchParentId,nameUnique,codeUnique,functionCodesMap } = roleGroup;
    const column=[
		{
            title: '角色组名称',
            dataIndex: 'name',
            ...
            render:(text,record,index)=>{
                return (
                    functionCodesMap['CF_MANAGE_RG_VIEW']?
			...
			render:(text,record,index)=>{
				return(
					<span>
						{functionCodesMap['CF_MANAGE_RG_VIEW']?<a onClick={()=>{
                                                    showViewWindow(record);
                                                }}>查看</a>:''}
						{functionCodesMap['CF_MANAGE_RG_UPDATE']?<span className="ant-divider" />:''}
						{functionCodesMap['CF_MANAGE_RG_UPDATE']?<a onClick={()=>{
                                                    showEditWindow(record);
                                                }}>修改</a>:''}
                        {functionCodesMap['CF_MANAGE_RG_DEL']?<span className="ant-divider" />:''}
                        <VtxPopconfirm title="确认删除此记录吗？" onConfirm={()=>{
                            dispatch({type:'roleGroup/deleteRoleGroup',payload:{id:record.id}});
                        }}>
                            {functionCodesMap['CF_MANAGE_RG_DEL']?<a>删除</a>:''}
                        </VtxPopconfirm>
					</span>
				)
			}
    ...
    const newItemProps = {
        modalProps:{
            ...
            <Button key="cancel" size="large" onClick={()=>{
                dispatch({type:'roleGroup/clearNewItem'});
            }}>清空</Button>,
            <Button key="submit" type="primary" size="large" loading={saving} onClick={()=>{
                dispatch({type:'roleGroup/updateNewItem',payload:{checkState: true}});
                dispatch({type:'roleGroup/addRoleGroup'});
            }}>
              保存
            </Button>,
            ],
        ...
        contentProps:{
            ...
            codeUnique,
            updateItem(obj){
                dispatch({type:'roleGroup/updateNewItem',payload:{
                    ...obj
                }})
            },
            validateRoleGroup(obj){
                dispatch({type:"roleGroup/validateRoleGroup",payload:{obj:obj}});
            }
        }
    }
    const editItemProps = {
        modalProps:{
            ...
            <Button key="submit" type="primary" size="large" loading={false} onClick={()=>{
                dispatch({type:'roleGroup/updateEditItem',payload:{checkState:true}});
                dispatch({type:'roleGroup/updateRoleGroup'});
            }}>
              保存
            </Button>,
            ],
        ...
        contentProps:{
            ...
            codeUnique,
            updateItem(obj){
                dispatch({type:'roleGroup/updateEditItem',payload:{
                    ...obj
                }})
            },
            validateRoleGroup(obj){
    ...
    const gridProps = {
        ...
        rowSelection:{
            ...
            onChange(selectedRowKeys){
                dispatch({type:'roleGroup/updateState',payload:{
    ...
    function showViewWindow(data){
        dispatch({type:'roleGroup/updateViewItem',payload:{
            ...data,
            visible:true
        }})
    }
    ...
    function bulkDelete(){
        if(selectedRowKeys.length==0){
            ...
            Modal.confirm({
                ...
                cancelText: '取消',
                onOk(){
                    dispatch({type:'roleGroup/deletesRoleGroup'});
                }
            });
        }
    }
		...
		<div className={styles.main}>
            ...
            <div className={styles.rigthCont}>
                <div className={styles.bt_bar}>
                    {functionCodesMap['CF_MANAGE_RG_ADD']?<Button icon="file-add" className='primary' onClick={showNewWindow}>新增</Button>:''}
...
export default connect(({roleGroup})=>({roleGroup}))(RoleGroup);
