...
function Tenantxzqh({dispatch,tenantxzqu}){
    ...
    const mapCallback2=(lglt)=>{
        ...
        dispatch({'type':'tenantxzqu/updateWindowMapInfo',payload:{
            showMap: true,
            mapLines: [{id:'pointt',paths:positionStr}],
        }});
        dispatch({type:'tenantxzqu/updateNewItem',payload:{
    			...
    			<div className={styles.leftTree}>
	             ...
	             <div className={styles.rigthCont}>
	             	<div className={styles.bt_bar}>
						{functionCodesMap['CF_MANAGE_TENANT_XZQH_ADD']?<Button icon="file-add" className='primary' onClick={showNewWindow}>新增</Button>:''}
						{functionCodesMap['CF_MANAGE_TENANT_XZQH_DEL']?<Button icon="delete" className='delete' onClick={deleteItems}>删除</Button>:''}
