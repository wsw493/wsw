...
function Dept({dispatch,dept}){
    ...
    const mapCallback = (lt,lglt)=>{
        ...
        dispatch({type:'dept/updateNewItem',payload:{address:lglt}});
        dispatch({type:'dept/updateEditItem',payload:{address:lglt}});
    }
    const closeMapModal = ()=>{
        dispatch({type:'dept/updateState',payload:{modal1Visible:false}});
    }
		...
		<div className={styles.main}>
			<div className={styles.contBox}>
	            ...
	            <div className={styles.bt_bar}>
	                {functionCodesMap['CF_MANAGE_TENANT_DEPT_ADD']?<Button icon="file-add" className='primary' onClick={showNewWindow}>新增</Button>:''}
	                {/* <Button icon="delete" className='delete' onClick={showNewWindow}>删除</Button> */}
	            </div>
