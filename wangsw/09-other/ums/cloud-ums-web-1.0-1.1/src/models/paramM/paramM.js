...
import {getParamTypeTable,getParamGroupTree,saveParamType,updateParamType,deleteParamType,
    validateParamType,getParamValueTable,saveParamValue,updateParamValue,deleteParamValue,getFunctionPermissionMap
...
export default {
  ...
  state: {
        ...
        functionCodes: ['CF_MANAGE_PARAM_TYPE_LIST','CF_MANAGE_PARAM_TYPE_DEL','CF_MANAGE_PARAM_TYPE_ADD','CF_MANAGE_PARAM_TYPE_UPDATE',
        'CF_MANAGE_PARAM_TYPE_VIEW','CF_MANAGE_PARAM_ADD','CF_MANAGE_PARAM_LIST','CF_MANAGE_PARAM_BAT_DEL','CF_MANAGE_PARAM_UPDATE'],
        functionCodesMap: {}
    },
  subscriptions: {
  ...
  effects: {
    ...
    *getParamTypeTableData({payload},{select,call,put}){
        const state = yield select(({param})=>param);
    ...
    *addParamType({ payload }, { call, put,select }){
        const {newItem,codeUnique,nameUnique}=yield select(({param})=>param);
        let formCheck = [
    ...
    *updateParamType({ payload }, { call, put,select }){
        const {editItem,codeUnique,nameUnique,defaultCloumns}=yield select(({param})=>param);
        let formCheck = [
    ...
    *getFunctionPermissionMap({payload},{select,call,put}){
        const {functionCodes} = yield select(({param})=>param);
    ...
    *getParamValueTableData({payload},{select,call,put}){
        ...
        const {valueItems} = yield select(({param})=>param);
        yield put({type:'updateValueItems',payload:{tableLoading:true,selectedRowKeys:[]}});
        const data = yield call(getParamValueTable ,{
    ...
    *addParamValue({ payload }, { call, put,select }){
        const {valueNewItems} = yield select(({param})=>param);
        const data = yield call(saveParamValue ,valueNewItems);
        if(data && data.result == 0){
    ...
    *updateParamValue({ payload }, { call, put,select }){
        const {valueNewItems,defaultValueCloumns} = yield select(({param})=>param);
        const data = yield call(updateParamValue ,handleEditItem(defaultValueCloumns,valueNewItems));
        if(data && data.result == 0){
            yield put({type:'getParamValueTableData'});
            message.success(data.msg,3);
        }else {
    ...
    *deletesParamValue({ payload }, {call, put, select}){
        const {valueItems} = yield select(({param})=>param);
        const data  = yield call(deleteParamValue, {ids:(valueItems.selectedRowKeys).toString()});
        if(data && data.result == 0) {
            message.success(data.msg,3);
            yield put({type:'updateValueItems',payload:{selectedRowKeys:[],currentPageIndex:1,
            pageSize:10}});
            yield put({type:'getParamValueTableData'});
        }else {
            if(data && data.exception){
