import {getDivisionTenantTable, saveDivisionTenant,updateDivisionTenant,deletesDivisionTenant,DivisionCommonCode,
  ...
  effects: {
  	/*获取列表数据*/
        ...
        *getDivisionTenantDtl({payload},{put,call,select}){
            const data=yield call(getDivisionTenantDtl,{id:payload.id});
            if(data && data.result==0){
        ...
        *addDivision({ payload }, { call, put,select }){
            const {newItem,searchTreeId}=yield select(({tenantxzqu})=>tenantxzqu);
	        let formCheck = [
        ...
        *updateDivision({ payload }, { call, put,select }){
            ...
            latitude,longitude,startTime,scope}=state.editItem;
	        let formCheck = [
	        ...
	        if(formValidation(formCheck)){
                ...
                const params={
                    ...
                    latitude,longitude,startTime,scope
                }
