...
export default {
    ...
    effects: {
        ...
        *addWorkElement({ payload }, { call, put,select }){
            const {newItem,codeUnique,nameUnique,defaultCloumns} = yield select(({workElementGis})=>workElementGis);
            let formCheck = [
            ...
            if(formValidation(formCheck) && !codeUnique && !nameUnique){
                ...
                const data  = yield call(saveWorkElement, handleEditItem(defaultCloumns ,newItem));
                if(data && data.result == 0) {
                    yield put({type:'updateState',payload:{
                        addPanelDisplay:{},
                        isDoEdit2:false,
        ...
        *updateWorkElementForm({ payload }, { call, put,select }){
            const {editItem,codeUnique,nameUnique,defaultCloumns} = yield select(({workElementGis})=>workElementGis);
            let formCheck = [
            ...
            if(formValidation(formCheck) && !codeUnique && !nameUnique){
                yield put({type:'updateState',payload:{saving:true}});
                const data  = yield call(updateWorkElementGis, handleEditItem(defaultCloumns ,editItem));
                if(data && data.result == 0) {
        ...
        *updateWorkElement({ payload }, { call, put,select }){
            ...
            const {defaultCloumns,newItem} = yield select(({workElementGis})=>workElementGis);
            const data  = yield call(updateWorkElementGis, handleEditItem(defaultCloumns,newItem));
            if(data && data.result == 0) {
        ...
        *getDtoById({ payload }, { call, put,select }){
            ...
            if(workElement){
                ...
                switch(shape){
                  ...
                  case "circle":
                    var arr = lonLatStr.split(",");
                    yield put({type:"updateState",payload:{mapCircles:[{id:"workE",longitude:arr[0],latitude:arr[1],radius:workElement.radius}],mapCenter:[arr[0],arr[1]]}});
                    break;
                  case "rectangle":
                    yield put({type:"updateState",payload:{mapPolygons:[{id:"workE",rings:splitOverride(lonLatStr)}]}});
                    break;
