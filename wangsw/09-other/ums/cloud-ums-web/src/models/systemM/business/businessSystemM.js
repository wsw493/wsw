...
import {
    getBusinessSystemTable,saveBusinessSystem,updateBusinessSystem,validateBusinessSystemAccount,
    validateBusinessSystem,getFunctionPermissionMap
...
export default {
  ...
  state: {
      ...
      functionCodes: ['CF_MANAGE_TENANT_BS_LIST','CF_MANAGE_TENANT_BS_ADD','CF_MANAGE_TENANT_BS_UPDATE','CF_MANAGE_TENANT_BS_VIEW'],
      functionCodesMap: {}
  },
  subscriptions: {
    setup({ dispatch, history }) {
            history.listen(({ pathname }) => {
                if (pathname === '/businesssystem') {
  ...
  effects: {
    ...
    *getTableData({payload},{select,call,put}){
        const state = yield select(({businessSystem})=>businessSystem);
        yield put({type:'updateState',payload:{tableLoading:true,selectedRowKeys:[]}});
        const data = yield call(getBusinessSystemTable ,{
    ...
    *addBusinessSystem({payload},{select,call,put}){
        const {newItem, nameUnique, codeUnique, passwordFlag} = yield select(({businessSystem}) => businessSystem);
        let formCheck = [
            {
                checkType: 'required',
        ...
        if (formValidation(formCheck) && !nameUnique && !codeUnique && !passwordFlag) {
            ...
            const data = yield call(saveBusinessSystem, newItem);
            if (data && data.result == 0) {
                yield put({
                    type: 'updateNewItem',
                    payload: {
    ...
    *updateBusinessSystem({payload},{select,call,put}){
        const {editItem, codeUnique, defaultCloumns} = yield select(({businessSystem}) => businessSystem);
        let formCheck = [
            {
                checkType: 'required',
        ...
        if (formValidation(formCheck) && !codeUnique ) {
            ...
            const data = yield call(updateBusinessSystem, handleEditItem(defaultCloumns, editItem));
            if (data && data.result == 0) {
                yield put({
                    type: 'updateEditItem',
                    payload: {
    ...
    *validateCode({payload},{select,call,put}){
        const data = yield call(validateBusinessSystem, {
            key: payload.obj.key,
            param: {
                ...payload.obj
            }
    ...
    *validateAccount({payload},{select,call,put}){
        const data = yield call(validateBusinessSystemAccount, {
            ...payload.obj
        });
    ...
    *getFunctionPermissionMap({payload},{select,call,put}){
        const {functionCodes} = yield select(({businessSystem})=>businessSystem);
