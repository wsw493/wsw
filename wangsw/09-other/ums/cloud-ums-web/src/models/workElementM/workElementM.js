...
import {getWorkElementTable, getWorkElementTree, saveWorkElement,getWorkElementDtoById,validateWorkElementCode,
        ...
        importWorkElementData,getImportTableData,getDivisionTenantTree,exportWorkelement,getLoginInfo,getParamsDone} from '../../services/remoteData';
...
export default {
    ...
    effects: {
        ...
        *addWorkElement({ payload }, { call, put,select }){
            yield put({type:'updateState',payload:{tableLoading:true}});
            const {newItem,codeUnique,nameUnique,mapType} = yield select(({workElement})=>workElement);
            let formCheck = [
            ...
            if(formValidation(formCheck) && !codeUnique && !nameUnique){
                ...
                const data  = yield call(saveWorkElement, {...newItem,mapType});
                if(data && data.result == 0) {
        ...
        *updateWorkElement({ payload }, { call, put,select }){
            yield put({type:'updateState',payload:{tableLoading:true}});
            const state = yield select(({workElement})=>workElement);
            const {codeUnique,nameUnique,mapType} = yield select(({workElement})=>workElement);
            ...
            if(formValidation(formCheck) && !codeUnique && !nameUnique){
                ...
                const data  = yield call(updateWorkElement, {...param,mapType});
                if(data && data.result == 0) {
        ...
        *getDtoById({ payload }, { call, put,select }){
            let mapPoints=[],
                ...
                mapCircles=[],
                mapCenter=[];
            const {operatedRow,mapType} = yield select(({workElement})=>workElement);
            const data  = yield call(getParamsDone, {id: operatedRow.id,mapType});
            if(operatedRow && data && data.result==0){
                var shape = operatedRow.shape;
                var lonLatStr = data.data;
                yield put({type:"updateEditItem",payload:{
                    paramsDone: data.data
                }});
                switch(shape){
                  case "point":
