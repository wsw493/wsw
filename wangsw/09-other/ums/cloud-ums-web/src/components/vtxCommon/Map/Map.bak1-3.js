...
class Map extends React.Component {
    ...
    */
    ...
    //数据解析(分析,新增,更新,删除对应的数据)
    arrayDataMatch(oldData,newData){
        let deletedData = Set(oldData).subtract(Set(newData)).toJS();
        let addedData = Set(newData).subtract(Set(oldData)).toJS();
        let updatedData = Set(oldData).intersect(Set(newData)).toJS();
        return {deletedData,addedData,updatedData};
    }
    deepEqual(a,b){
    ...
    componentWillReceiveProps(nextProps) {//已加载组件，收到新的参数时调用
        ...
        */
        if(mapPoints instanceof Array && !t.deepEqual(mapPoints,t.props.mapPoints)){
            //过滤编辑的图元
            let oldMapPoints = t.props.mapPoints.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
            let newMapPoints = mapPoints.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
        ...
        */
        if(mapLines instanceof Array && !t.deepEqual(mapLines,t.props.mapLines)){
            let oldMapLines = t.props.mapLines.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
            let newMapLines = mapLines.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
        ...
        */
        if(mapPolygons instanceof Array && !t.deepEqual(mapPolygons,t.props.mapPolygons)){
            let oldMapPolygons = t.props.mapPolygons.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
            let newMapPolygons = mapPolygons.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
        ...
        */
        if(mapCircles instanceof Array && !t.deepEqual(mapCircles,t.props.mapCircles)){
            let oldMapCircles = t.props.mapCircles.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
            let newMapCircles = mapCircles.filter((item)=>{return mapEditIds.indexOf(item.id) == -1});
        ...
        //图元编辑调用
        if(mapEditIds instanceof Array && !!mapEditIds[0] && !t.deepEqual(mapEditIds,t.props.mapEditIds)){
            let {deletedData:endIDs,addedData:editIds} = t.arrayDataMatch(t.props.mapEditIds,mapEditIds);
            editIds.map((item)=>{
                t.doEdit(item);
            })
            endIDs.map((item)=>{
