...
import {VtxMap,VtxGrid} from 'vtx-ui';
...
const WorkElement = ({dispatch,workElement}) => {
    ...
    radioG,importDetail,importDelWindow,codeUnique,nameUnique,mapCenter,setCenter,mapType,setVisiblePoints}=workElement;
    let myForm = null;
    const columns = [
		{
			title: '编码',
		...
		<div className={styles.main}>
            ...
            >
                ...
                <Select value={searchWorkElementTypeId} onSelect={(value, option)=>{
                    ...
                    {workElementTypeList.map((element,index)=>{return <Option value={element.id} key={index} name="searchWorkElementTypeId">{element.name}</Option>;})}
                </Select>
                <TreeSelect
			...
			<div className={styles.bt_bar}>
                ...
                {/* <Button icon="eye-o" onClick={showViewWindow}>查看</Button>
				<Button icon="edit" onClick={showEditWindow}>更新</Button> */}
				<Button icon="delete" type="Button" onClick={deleteItem}>删除</Button>
				<Button icon="login" type="Button" onClick={()=>{
                    dispatch({type:'workElement/updateState',payload:{importWindow:true}});
                }}>导入</Button>
                <Button icon="logout" type="Button" onClick={()=>{
