...
const WorkElementGis = ({dispatch,orgWorkElementTree,orgTreeData,divisionTreeData,mapPanelPosition,editWindow,newItem,workElementTypeList,searchWorkElementTypeId,
		...
		mapCenter,checkedWorkElementList,codeValidatFlag,mapCenter2,setCenter,setCenter2,setVisiblePoints,mapVisiblePoints,initWorkElementKeys,checkedKeys_elem,mapType,
	...
	const saveMapData = (data)=>{
		const type = data&&data.geometry?data.geometry.type:'';
		var lonlatArr = '';
		var positionStr = '';
		var latitudes='';
		var longitudes='';
		switch(type) {
			case 'point':
				...
				dispatch({type:'workElementGis/updateState',payload:{mapPoints2:[{
					id: data.id,
			...
			case 'polyline':
				...
				dispatch({type:'workElementGis/updateState',payload:{mapLines2:[...mapLines2,{
					id: data.id,
			...
			case 'circle':
				...
				dispatch({type:'workElementGis/updateState',payload:{mapCircles2:[...mapCircles2,{
					id: data.id,
					longitude: data.geometry.x,
					latitude: data.geometry.y,
			...
			case 'rectangle':
				...
				dispatch({type:'workElementGis/updateState',payload:{mapPolygons2:[...mapPolygons2,{
					id: data.id,
		...
		<div className={style.main}>
			<div className={style.fullMap}>
				<VtxMap 
					...
					drawEnd={(obj)=>{
						dispatch({type:'workElementGis/updateState',payload:{
							editGraphicId2:obj.id
						}});
						saveMapData(obj);
						setTimeout(()=>{
							dispatch({type:'workElementGis/doEdit',payload:{
								editGraphicId: obj.id
							}});
						},1);
					}}
					isRemove = {isRemove}
			...
			<div className={style.data_panel} style={{left:panelStatus=='open'?30:-300}}>
				<div className={style.operate_search_panel}>
					...
					<div><label>图元类型：</label><Select value={searchWorkElementTypeId} onSelect={(value, option)=>{
							...
							{
								workElementTypeList.map((element)=>{
									return <Select.Option value={element.id} key={element.id} name="searchWorkElementTypeId">{element.name}</Select.Option>;
								})
							}
						</Select></div>
					<Button onClick={()=>{
				...
				<div className={style.operate_tree_panel} style={{height: window.innerHeight*0.62}}>
				...
				<span className={panelStatus=='open'?(style.expendBtn + ' iconfont icon-arrow-left'):(style.expendBtn + ' iconfont icon-arrow-right')} onClick={()=>{
					dispatch({type:'workElementGis/updateState',payload:{panelStatus: panelStatus=='open'?'close':'open'}});
				}}></span>
			</div>
			...
			<div className={style.opBtnGroup}>
				...
				{/* <Input value={inputVal} style={{width:'150px',marginLeft:'15px'}} onChange={(e)=>{
				...
				<div className={style.content}>
					...
					<div className={style.inputItem}>
                        ...
                        validated={newItem.checkState?(validate(['required'],newItem.workElementTypeId)==''):true}>
							{
								workElementTypeList.map((element)=>{
									return <Select.Option value={element.id} key={element.id} shape={element.shape}>{element.name}</Select.Option>;
								})
							}
						</StateSelect>
						<Button className={style.editWorkBtn} onClick={()=>{
							if(newItem.shape == ""){
                                  ...
                                  const mapDrawConfig = {
                                        geometryType: newItem.shape=='line'?'polyline':newItem.shape,
                                        parameter: {},
                                        data: {id: 'workE'}
                                  };
