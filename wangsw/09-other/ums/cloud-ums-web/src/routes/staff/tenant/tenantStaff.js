    		...
    		<div className={style.rigthCont}>
                ...
                >
                    ...
                    <Select style={{ width: '100%' }} value={educationId} onChange={queryEducationChanged}>
                        ...
                        {
                            baseData.STAFF_EDUCATION.map((item,index)=>{
                    ...
                    <Select style={{ width: '100%' }} value={postId} onChange={queryPartyPostChanged}>
                        <Option value=''>无</Option>
                        {
                            baseData.STAFF_POST.map((item,index)=>{
                                return <Option value={item.value} key={index}>{item.text}</Option>
                            })
                        }
                    </Select>
    			...
    			<div className={style.bt_bar}>
    				{functionCodesMap['CF_MANAGE_STAFF_ADD']?<Button icon="file-add" onClick={showNewWindow}>新增</Button>:''}
    				{functionCodesMap['CF_MANAGE_STAFF_UPDATE']?<Button icon="edit" onClick={showEditWindow}>修改</Button>:''}
                    {functionCodesMap['CF_MANAGE_STAFF_VIEW']?<Button icon="eye-o" onClick={showViewWindow}>查看</Button>:''}
    				{functionCodesMap['CF_MANAGE_STAFF_DELETE']?<Button icon="delete" onClick={bulkDelete}>删除</Button>:''}
