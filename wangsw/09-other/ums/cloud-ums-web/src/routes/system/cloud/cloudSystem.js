...
import {VtxSearchMap,VtxGrid} from 'vtx-ui';
function CloudSystem({dispatch,cloudSystem}) {
	const {tableData,tableLoading,pageSize,currentPageIndex,newItem,editItem,viewItem,cloudSystemName,enabled,
        selectedRowKeys,mapCenter,modal1Visible,saving,totalItems,windowMapInfo,systemName,systemCode,
    ...
    const querySystemCodeChanged = (e)=>{
    	dispatch({type:'cloudSystem/updateState',payload:{systemCode:e.target.value}});
    }
	return (
		<div className={styles.main}>
            <div className={styles.bt_bar}>
                {functionCodesMap['CF_MANAGE_CS_ADD']?<Button icon="file-add" onClick={showNewWindow}>新增</Button>:''}
            </div>
            <VtxGrid 
            titles={['系统编码','系统名称']}
            gridweight={[1, 1]}
            ...
            clear={clearQuery}
            >
                <Input value={systemCode} onChange={querySystemCodeChanged} placeholder='输入系统编码'/>
