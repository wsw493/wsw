 ...
 * @version 0.0.1
 * @since Feb 19, 2011
 * @功能：实现坐标转换功能 -- 经纬度投影到高斯坐标系 @说明：
 *              http://blog.sina.com.cn/s/blog_5656b0020100chbv.html
 */
public class CoordinateConversion {
	/**
	 * @author lxw @功能：由经纬度反算成高斯投影坐标【最终结果以米为单位】 @说明：
	 */
	public static APoint BLToGauss(double longitude, double latitude) {
		APoint point = new APoint();

		// 取得每0.01纬度的米数
		double lngMileage = Basic.getDistance(longitude, latitude, longitude + 0.01, latitude, CoordinateEnum.WGS84);
		// 取得每0.01经度的米数
		double latMileage = Basic.getDistance(longitude, latitude, longitude, latitude + 0.01, CoordinateEnum.WGS84);

		point.setX(longitude * lngMileage * 100);
		point.setY(latitude * latMileage * 100);
		return point;

	}

	/**
	 * @author lxw @功能：由WGS84经纬度反算成高斯投影坐标【最终结果以米为单位】 @说明：
	 */
	...
	// 由高斯投影坐标反算成经纬度 没测试过
	public static double[] GaussToBL(double X, double Y, CoordinateEnum coordinate)// ,
																					// double
																					// *longitude,
	// double *latitude)
	{
		...
		latitude1 = fai - (NN * Math.tan(fai) / R)
				* (D * D / 2 - (5 + 3 * T + 10 * C - 4 * C * C - 9 * ee) * D * D * D * D / 24 + (61 + 90 * T + 298 * C + 45 * T * T - 256 * ee - 3 * C * C) * D * D * D * D * D * D / 720);
		// 转换为度 DD
