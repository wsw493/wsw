...
public class CloudStaffTenantController extends BaseController {
	...
	private CloudStaffSearchDto getSearchCondition(HttpServletRequest request) throws Exception {
		...
		logger.info("pageList(" + "tenantId=" + tenantId + ", departmentId=" + departmentId + ", selectedId=" + selectedId + ", selectedType=" + selectedType + ", code=" + code + ", name=" + name
				+ ", socialSecurityNo=" + socialSecurityNo + ", credentialNum=" + credentialNum + ")");
	...
	public RestResultDto<Boolean> download(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			 ...
			 * columnFields.lastIndexOf(",")); columnNames =
			 * columnNames.substring(0, columnNames.lastIndexOf(","));
			 */
			...
			if (isDownloadAll) {
				 ...
				 * searchFilter.add(new SearchFilter("id", Operator.IN,
				 * StringUtil.splitComma(downloadIds))); } searchFilter =
				 * CommonUtils.bindTenantId(searchFilter);
				 * andSearchFilters.addSearchFilter(searchFilter);
				 * 
				 * list = cloudStaffService.findListByFilters(andSearchFilters,
				 * defSort);
				 */
				CloudStaffSearchDto searchDto = new CloudStaffSearchDto();
				searchDto.setTenantId(tenantId);
				...
				/*
				 * Pageable pageable = ForeContext.getPageable(request,
				 * defSort); Page<CloudStaff> pageResult =
				 * cloudStaffService.findPageByFilters(pageable,
				 * andSearchFilters.addSearchFilter(searchFilter));
				 */
				if (null != pageResult) {
